...
- OTBR is pinned to `openthread/border-router:sha-8ee13eb`; `sha-b0399ae` exits against the existing Thread RCP with `otPlatRadioSetMacKey() ... InvalidArgument`.
...
- `pylon.magicsk.eu` is currently a DNS-only A record for the Oracle ingress `132.226.217.72`, whose `ens3` DNAT forwards to Caddy on `magic-pylon`. The direct origin IPv6 `2a01:c846:3901:9301::a` was black-holed in both directions on 2026-07-27; restore its AAAA record only after both outbound IPv6 and an external inbound HTTPS probe succeed.
...
- If codex-wrapper reports that its access token cannot be refreshed, `codex login status` may still misleadingly say it is logged in. Refresh it with the bundled CLI under `homelab.user`, setting `HOME` to the service `dataDir` and `CODEX_HOME` to `${dataDir}/codex`, then run `login --device-auth`.
- Open WebUI must keep its systemd address families restricted to `AF_INET`/`AF_UNIX`: its startup performs a blocking remote fetch before binding port 8086, and black-holed host IPv6 otherwise leaves `chat.magicsk.eu` returning 502 while the unit still appears active.
...
- Cloudflare-proxied web records should target the separate `pylon.magicsk.eu` origin name rather than the apex `magicsk.eu`; this keeps origin failover centralized even while both names use the Oracle ingress IPv4. `reciper.magicsk.eu` previously returned intermittent Cloudflare `522` when it targeted the apex directly.
...
- Redlib uses the locally source-built image `localhost/redlib:a4d36e954cf1` from upstream commit `a4d36e954cf1`, built with `Dockerfile.ubuntu`; the published Quay image currently uses a broken build path, while the nixpkgs build lags the `wreq` TLS-fingerprint fix. It bypasses the VPN on subnet `172.30.16.0/24` because Reddit blocks the VPS public IP. Build and validate a replacement local image before changing the pinned tag.
...
- Paperless-ngx v3 requires `PAPERLESS_SECRET_KEY`; keep it in the persistent `${homelab.mounts.config}/paperless/secrets.env` runtime env file, not in the Nix store. The v3 config explicitly keeps duplicate rejection enabled, while its built-in ignore patterns cover `.DS_Store` and `desktop.ini`.
...
- Remux uses bypass subnet `172.30.17.0/24` so AIOStreams and media/debrid traffic leave through the home connection.
- CrossWatch uses bypass subnet `172.30.18.0/24` so media-server and tracker API traffic leaves through the home connection.
...
- Podman containers that perform large outbound transfers or arbitrary public web fetches should use a bypassed network. The current bypassed `/24` subnets are `172.30.12.0/24` for flaresolverr, `172.30.13.0/24` for paperless, `172.30.14.0/24` for changedetection.io, `172.30.15.0/24` for html2rss-web, `172.30.16.0/24` for redlib, `172.30.17.0/24` for Remux, and `172.30.18.0/24` for CrossWatch; choose the next free `172.30.x.0/24` (next is `172.30.19.0/24`) for another bypassed network.
- Adding a *new* bypassed Podman network through `nixos-rebuild switch` (no reboot) can leave `aardvark-dns` not listening on the new gateway `172.30.<n>.1:53`, so containers on it fail DNS with `Could not resolve host` (e.g. redlib logs `Failed to create OAuth client before timeout`). Fix once with `sudo podman network reload --all` (rebinds every gateway); a reboot also fixes it because all networks are created before their containers start. If Podman 5.8.2 reports Netavark `sequence_number out of sync`, do not keep retrying `network reload --all`: reboot or recreate only the affected ephemeral network, then verify its gateway address and `:53` listener. Verify bound gateways with `sudo ss -ulnp | grep ':53 '`.
